...
class ImageProcessor:
    ...
    def __init__(self, config: Dict):
        ...
        self.remover = BackgroundRemover(model_name=config.get('model', 'u2net'))
        self.replacer = None
        ...
        if bg_path and os.path.exists(bg_path):
            ...
            logging.warning("No valid background image found. Will create default background.")
    ...
    def _create_default_background(self, size: tuple) -> Image.Image:
        ...
        for y in range(size[1]):
            gray_val = int(180 + (y / size[1]) * 40)  # Gradient from 180 to 220
    ...
    def process_single_image(self, input_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            logging.info(f"Processing: {os.path.basename(input_path)} ({original_size[0]}x{original_size[1]})")
            ...
            logging.error(f"✗ Failed to process {os.path.basename(input_path)}: {str(e)}")
            return False
    ...
    def process_batch(self, input_folder: str, output_folder: str) -> Dict:
        ...
        # Print header
        print(f"\n{Fore.CYAN}{'='*50}")
        print(f"{Fore.CYAN}Image Background Transformer")
        ...
        for idx, input_path in enumerate(tqdm(image_files, desc="Processing", unit="image")):
            ...
            if self.config.get('skip_existing', True) and os.path.exists(output_path):
                logging.info(f"Skipping existing: {os.path.basename(output_path)}")
            ...
            if success:
                ...
                tqdm.write(f"{Fore.GREEN}[{idx+1}/{len(image_files)}] ✓ {os.path.basename(input_path)} ({elapsed:.1f}s){Style.RESET_ALL}")
            else:
                self.stats['failed'] += 1
                tqdm.write(f"{Fore.RED}[{idx+1}/{len(image_files)}] ✗ {os.path.basename(input_path)} (failed){Style.RESET_ALL}")
